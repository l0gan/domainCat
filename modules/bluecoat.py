...
class Bluecoat:
    def check_category(self, domain):
        ...
        data = json.dumps({'url':domain,'captcha':''}).encode("utf-8")
        # data = urllib.parse.urlencode(json.dumps({'url':domain,'captcha':''})).encode("utf-8")
        # request = urllib2.Request("https://sitereview.bluecoat.com/resource/lookup", data=data)
        request = urllib.request.Request("https://sitereview.bluecoat.com/resource/lookup", data=data)
